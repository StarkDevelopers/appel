...
class FileUpload extends React.Component {
  ...
  uploadFile() {
    ...
    fetch(`/api/upload-file/${this.props.roomName}`, {
        method: 'POST',
        body: formData
      })
      ...
      .then(data => {
        if (data.filename) {
          this.props.onUploaded(this.state.file.name, data.filename, this.state.fileSize);
        }
        this.setState({
          file: null,
          fileSize: null
      ...
      .catch(error => {
        ...
        this.setState({
          showAlert: true,
          showAlertMessage: `Error while uploading file`
        });
        this.setState({
          file: null,
          fileSize: null
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(style)(FileUpload));
